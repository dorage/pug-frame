...
- **상단 헤더** — `리셋`, `URL 복사`(현재 코드가 담긴 공유 URL 을 클립보드로 복사).
  - `리셋` 버튼은 클릭하면 기본 데모로 되돌리고, **마우스를 올리면(hover) 데모 선택 드롭다운**이 열린다. 키보드 포커스로도 열린다.
  - 드롭다운 항목을 고르면 해당 데모 소스로 에디터와 미리보기가 즉시 교체된다.
  - 제공 데모: `기본`(2-화면 내비게이션 예시), `인스타그램`(피드 → 게시물 상세), `와츠앱`(채팅 목록 → 대화). 각 데모는 `p-focus` 로 화면을 이동한다.
