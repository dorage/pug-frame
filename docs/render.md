...
- 와이어프레임 요소 키워드(`circle`/`image`)는 `.pf-*` 클래스 div로 매핑된다.
- 그 외 리프 태그(`div`, `section`, `button` 등)와 `#id`·attribute는 Pug가 처리하는 대로 보존된다.
...
- `p-icon`은 **컨텐츠** attribute로, 렌더 단계에서 lucide 인라인 SVG로 치환되어 정적/임베드 출력 양쪽에 남는다.
...
- 요소 자리표시자: `.pf-circle`(정원 아바타), `.pf-image`(회색 이미지 박스).
...
3. `resolveIcons`: `p-icon`을 가진 요소에 lucide 인라인 SVG를 삽입한다(attribute 제거 전에 수행하므로 정적/임베드 양쪽에 남는다).
...
- `ELEMENT_CLASSES` (object): 요소 키워드(`circle`/`image`) → `.pf-*` 클래스 맵. 확장 지점.
